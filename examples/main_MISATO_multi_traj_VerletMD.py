...
def evaluate(loader):
    ...
    for batch_idx, batch in enumerate(L):
        ...
        pos_input = batch.ligand_trajectory_pos[:, 0, :].clone().detach()
        vel_input = batch.ligand_trajectory_pos[:, 1, :].clone() - batch.ligand_trajectory_pos[:, 0, :].clone().detach()
        trajectory_pred = []
        
        for traj_idx in range(0, 99):
            pos_target = batch.ligand_trajectory_pos[:, traj_idx+1, :].detach()
            ...
            condition_protein = (pos_N, pos_Ca, pos_C, batch.protein_backbone_residue, batch.batch_residue)
            condition = condition_ligand + condition_protein
            condition = [x.detach() for x in condition]
            pos_pred, vel_pred = binding_model.move([pos_input, vel_input], condition)

            pos_pred = pos_pred.detach()
            vel_pred = vel_pred.detach()

            trajectory_pred.append(pos_pred.detach().cpu())
            rmse_loss = rmse_loss + torch.sum((pos_pred - pos_target).pow(2).sum(dim=1).sqrt()).detach().cpu().item()
            mae_loss = mae_loss + torch.sum(torch.abs(pos_pred - pos_target)).detach().cpu().item()
            eval_count += pos_pred.shape[0]

            pos_input = pos_pred.clone()
            vel_input = vel_pred.clone()

        trajectory_target = batch.ligand_trajectory_pos.detach().cpu()[:, 1:, :].transpose(0, 1)
